...
<head>
  ...
  <script>
    $(document).ready(function(){
        ...
        // 영상에 대한 처리
        $.ajax({
          url:'http://182.231.5.187:8000/youtube?link=${param.url}',   // 추후 변경 필요
          type:'get',
          xhrFields:{
            responseType:'blob'
          },
    ...
    function subtitleRequest(){
      ...
      $.ajax({
        <cr:if test="${!empty param.url}"> url: 'http://182.231.5.187:8000/youtube/subtitle_download?link=${param.url}', </cr:if>
        <cr:if test="${!empty param.limited_hash}"> url: 'http://182.231.5.187:8000/upload/subtitle_download?limited_hash=${param.limited_hash}', </cr:if>   // 추후 변경 필요
        type: 'get',
        xhrFields: {
          responseType: 'blob'
        },
